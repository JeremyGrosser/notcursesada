...
package NC.Keys is
   ...
   --  representative of some Unicode. This covers both keyboard and mouse
   --  events, as well as signals and even window events.

   --  Rather than using one of the Private Use Areas of Unicode, we use the
   --  area beyond the 17 65536-entry Planes (1114112). We round up to 5000 so
   --  that it's trivial to identify synthesized characters based on their
   --  numeric definition here. This is safe, since we needn't convert these
   --  synthesized characters into UTF8 (they would otherwise require more than
   --  four bytes).

   PRETERUNICODEBASE : constant := 1115000;
